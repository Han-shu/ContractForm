...
function init_provisional_parameters(set::sets)
    ...
    CAPACITY = Dict("NUC" => 46.21849367289739, "CCGT" => 96.57505754304289, "WIND" => 131.5278712269312)
...
function make_cons_model(set::sets, param::parameters, prov_param::provisional_parameters)
    ...
    @constraint(cons_model, vol_cons["OPTION_1000"]==148.5)
...
function solve_credit_wind(set::sets, param::parameters, prov_param::provisional_parameters)
    ...
    for t in set.TIME_BLOCKS, f in set.FUEL_PRICES, r in set.PROFILES, s in set.SHIFTERS
        if prov_param.ENERGY_PRICE[t,f,r,s]>200
            index = index + 1
...
function solve_equilibrium(set, param, initial)
    FPATH = "../mandatory/beta0.8/intermediate_equ.json"
    finish = false
    ...
    # GUROBI_ENV = Gurobi.Env()
        ...
        if initial ==1  #initialize parameters
                ...
                open("../mandatory/beta0.8/max_risk_imbalance.json", "w") do f
                    println(f, "Running mandatory trading... ... ")
                end
                open("../mandatory/beta0.8/GEN_objective.json", "w") do f
                    println(f, "Running mandatory trading... ... ")
                end
            else
    ...
    while !finish && prov_param.ITERATION < 50000 
        ...
        open("../mandatory/beta0.8/max_risk_imbalance.json", "a") do f
            println(f, prov_param.ITERATION, " \t", max_risk_imbalance)
        end
        open("../mandatory/beta0.8/GEN_objective.json", "a") do f
...
OUTPUT_FPATH ="../mandatory/output_beta0.8.json"
