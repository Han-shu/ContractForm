...
function init_evaluation(param::Dict)
    ...
    GEN_CVAR, CONTRACT_PRICE, AVG_PAYOUT, RISK_PREMIA, Unserved_energy, AVG_prod_price, AVG_risk_price, AVG_spot_price, AVG_cons_price, spot_price_std, CONS_price_std, AVG_Profit, EXP_PROD, UNIT_RISK_COST, PAYOUT)
end
...
function init_provisional_parameters(set::sets)
    ITERATION = 1
    CAPACITY = Dict("NUC" => 32.02205838057366, "CCGT" => 106.74745627303116, "WIND" => 157.3098618484082)
...
function make_dispatch_model(set::sets, param::parameters, prov_param::provisional_parameters)
    ...
    @variable(dispatch_model, dem_served_fix[set.TIME_BLOCKS, set.FUEL_PRICES, set.PROFILES, set.SHIFTERS] >=0)
    @variable(dispatch_model, 0 <= dem_served_flex[set.TIME_BLOCKS, set.FUEL_PRICES, set.PROFILES, set.SHIFTERS] <= param.LOAD_FLEX)
    @variable(dispatch_model, prod[set.GENS, set.TIME_BLOCKS, set.FUEL_PRICES, set.PROFILES, set.SHIFTERS] >=0)
...
function calc_vol_limits(set, prov_param)
    ...
    for c in set.CONTRACTS
        ...
        for g in set.GENS
            vol_min_gen[g,c] = -2.0 * prov_param.CAPACITY[g]
        end
    end
...
function calc_payout(set, param, prov_param, option_price = 1000.0, future_price = 50.0, unit_cont_price = 50.0)
    ...
    for f in set.FUEL_PRICES, r in set.PROFILES, s in set.SHIFTERS
        ...
        payout["FUTURE_50",f,r,s] = sum(param.DURATION[t] * (prov_param.ENERGY_PRICE[t,f,r,s] - future_price) for t in set.TIME_BLOCKS)
        payout["UNIT_CONT",f,r,s] = sum(param.DURATION[t] * param.AVAILABILITY["WIND",r,t] * (prov_param.ENERGY_PRICE[t,f,r,s] - unit_cont_price) for t in set.TIME_BLOCKS)
    end
...
function solve_equilibrium(set, param, initial)
    FPATH = "../unrestricted/beta0.8/intermediate_equ.json"
    finish = false
    ...
    ϵ = 1
        ...
        if initial ==1  #initialize parameters
                ...
                open("../unrestricted/beta0.8/max_risk_imbalance.json", "w") do f
                    println(f, "Running unrestricted trading ... ... ")
                end
                open("../unrestricted/beta0.8/GEN_objective.json", "w") do f
                    println(f, "Running unrestricted trading ... ... ")
                end
        else
    ...
    while !finish && prov_param.ITERATION < 40000 
        ...
        while imbalance > ϵ
            ...
            prov_param.CONTRACT_PRICE = Dict(c => prov_param.CONTRACT_PRICE[c] + 5.0 * difference[c] for c in set.CONTRACTS)
            println("CONTRACT_PRICE is ", prov_param.CONTRACT_PRICE)
        ...
        open("../unrestricted/beta0.8/max_risk_imbalance.json", "a") do f
            println(f, prov_param.ITERATION, " \t", max_risk_imbalance)
        end
        open("../unrestricted/beta0.8/GEN_objective.json", "a") do f
...
OUTPUT_FPATH = "../unrestricted/output_beta0.8.json"
